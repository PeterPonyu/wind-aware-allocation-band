...
in the Results remain emitted from the hash-bound evidence tables.
\input{../revision/parameter_and_compatibility_table}
\input{../revision/campaign_design_table}
...
the resulting targets with one simulator-stock \texttt{DSLPIDControl} instance per
airframe.

The controller gains, motor model, PWM/RPM conversion and saturation path are
...
The revised support receipt keeps its inferential unit explicit: crossed-layout
...
the bound receipt. The primary seven-force family remains governed by the
...
bound receipt, checked for its declared row count and pairing, and emitted by the
...
not done. The crossed receipt does not add a signed-rank $p$ value to this
interval readout; its Holm family metadata is retained for the separate endpoint
audit.

\subsection{Reproducibility}

No number in this manuscript is typed by hand. Every episode log and derived table
this study rests on is recorded with its SHA-256 digest in a manifest that
accompanies the manuscript source. The figure and table code verifies each digest
against the bytes on disk before it reads them and aborts on any mismatch, and it
emits both the figures and the numeric macros that the text prints. A changed
artifact therefore breaks the build rather than silently changing a sentence.
This separation of source, computation and rendered claim follows
reproducible-research and simulation-reporting guidance~\cite{peng2011repro,sandve2013repro,
