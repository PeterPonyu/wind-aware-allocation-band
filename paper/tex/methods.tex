...
is therefore characterized as the operating envelope of the declared
rigid-body simulation contract and serves as the computational benchmark
against which an open-air extension is validated.
...
parameter contract. Read from that source, the stock integral gain is
\StockKi{}~N/m/s, the wind-up time $K_p/K_i$ is \IntegralTauS{}~s, the
horizontal integral clamp is \IntegralClampXy{}~m/s, the episode horizon is
\EpisodeHorizonS{}~s, and the largest steady integral force
$K_i\times\mathrm{clamp}$ is therefore \MaxSteadyIntegralForceN{}~N. This
stock position layer is consistent with the standard
...
holding the underlying stabilizing controller fixed, except in the
controller-identification audit that deliberately changes Ki, clamp and
horizon.
...
position controller in each arm. Here $\lambda=\LambdaReference{}$ in the aware arm and $\gamma$ is the tested target
feedforward gain (the inherited value is \FeedforwardGainDefault{}~m/N). A
pre-declared sweep substitutes \LambdaGridSize{} values of $\lambda$ at run
time on the same seeds, forces and frozen sources and is reported in
Results. The assignment is re-solved once per
...
episode rows are retained for audit and visualization and are not treated as
independent samples. The exact test, interval construction and multiplicity rule
are stated once, with the statistical analysis.
...
numbers admits the same \NRepeats{} pairs. The tolerance $\varepsilon = \RepeatTol{}$
degrees was not fixed in advance; it is a post-hoc reading rule adopted for this
repeat reading after the record was inspected, and we report the interval beside
it because Proposition~\ref{prop:tol} makes the result identical for every
tolerance in that gap.
