...
rejects that allocation hypothesis on the present benchmark and identifies a
different mechanism: target-position feedforward against a slow stock integral.
...
is a scoped identification rather than a measurement of allocation leverage:
against the stock slow integral (\StockKi{}~N/m/s) and the \EpisodeHorizonS{}~s
horizon, target-position feedforward changes short-episode completion.
Assignment is inert at the inherited $\lambda=0.25$ only; no $\lambda$ sweep was
run. Doubling that stock Ki collapses the 0.15~N paired advantage to
\AuditKiTenGain{}. A complete assignment-by-feedforward factorial attributes the
gain to the feedforward path. Allocation taxonomies and aerial-swarm surveys
still supply language for the rejected hypothesis~\cite{gerkey2004formal,
dias2006market,korsah2013taxonomy,chung2018swarm,skaltsis2023uavallocation}.
...
is a research-object schematic, not a fabricated flight photograph.
...
continuous information-to-completion trace. This remains a computational
baseline: the tested simulator envelope is the calibration reference, not a
hardware-general principle.
...
declared 0.05~N force grid; the controller-identification subsection below
shows how the same endpoint moves when those settings change.
...
The primary ladder is a measurement of the stock controller, not a property of
assignment. The stock position layer uses integral gain \StockKi{}~N/m/s,
...
The assignment path does not carry the gain. On the aware arm, the initial
assignment differs from Euclidean in \AuditInitialDivergence{} of
...
0.15~N. That inertness is at the inherited penalty weight $\lambda=0.25$
only; no $\lambda$ sweep was run.
...
The capture-radius fingerprint is likewise not a unit-slope saturation
curve. Baseline $F_{50}$ against capture radius has slope
\AuditRadiusSlope{}~N/m (interval [\AuditRadiusSlopeLo{},
\AuditRadiusSlopeHi{}]), which excludes 1.
...
integral inside a \EpisodeHorizonS{}~s episode. Prediction P1 is
\AuditPOneOverall{}: doubling Ki collapses the 0.15~N advantage, which
confirms that half of the pre-declaration, while clamp 4 and clamp 8 leave
\AuditClampChanged{} of the 0.15~N completions unchanged at the stock
horizon and therefore refute the clamp half at $T=\EpisodeHorizonS{}$~s.
The same clamp \AuditClampMattersLongHorizon{} matter at 12~s. Assignment
inertness is measured at $\lambda=0.25$ only. These bounds keep the
operating envelope a property of the stock integral and the short horizon,
not a general allocator law.

The assignment path does not carry the headline gain. Initial assignment
differs from Euclidean in \AuditInitialDivergence{} of \AuditAwareEpisodes{}
episodes, while used (replan) assignments diverge in \AuditUsedDivergence{}
episodes. The complete factorial localizes the completion change to
target-position feedforward; the assignment main effect and the
assignment-by-feedforward interaction stay within \FactorialOtherAbsMax{}
completion of zero. That inertness is at $\lambda=0.25$ only; no $\lambda$
sweep was run, so the paper does not promote an allocator law. Support
campaigns keep the same paired estimand: relative noise through
...
across capture radii keeps a transition of comparable size but moves it
from \RadiusTightPeakForce{} to \RadiusWidePeakForce{}~N, so
\KneeLevel{}~N is the transition of this operating point rather than a
constant of an allocator.

The remaining campaigns are support, not a second paper. Geometry contrasts
and the descriptive safety record stay in Results; they do not reopen an
allocation--control interface as the question this manuscript answers.
Nothing measured here is a field validation. Heterogeneous vehicles,
obstacle topology, a $\lambda$ sweep and field-sensor traces remain named
transfer axes rather than completed work. The present envelope is a
computational calibration reference against the stock integral, the
\EpisodeHorizonS{}~s horizon and the declared capture radius.

\section{Conclusions}

Target-position feedforward compensates a slow stock integral inside a
\EpisodeHorizonS{}~s episode. That scoped positive claim locates the
operating envelope: doubling \StockKi{} collapses the 0.15~N paired
advantage to \AuditKiTenGain{}, prediction P1 is \AuditPOneOverall{}, and
assignment remains un-swept in $\lambda$. The paper does not claim a
general allocator property, a hardware principle or a field result.
