...
choi2009cbba,korsah2013taxonomy,khamis2015mrta,chakraa2023mrta}; end-to-end UAV assignment and intercept studies
...
closest-approach companion is carried in Fig.~\ref{fig:miss} and is read alongside
the thresholded endpoint.
...
completion here; Panels A and B of Fig.~\ref{fig:radius} rerun the ladder at two further radii and
...
\NReproFields{} recorded fields, so the hash-bound record and its generating code
agree. The crossed record below therefore extends the same validated endpoint.
...
Fig.~\ref{fig:radius}A the paired advantage at each radius. The receipt corrects
...
separations were reached on a four-airframe simulated task whose objective carries
no collision constraint and whose physics branch models no inter-airframe
downwash, at mean separations of roughly two hull diameters, which is inside the
regime where rotor wake would dominate a physical flight. Only the scalar
three-dimensional separation is logged, so a pair separated mostly in altitude
could clear the hull at these distances and the counts above are an upper bound on
contacts. An implementation that adopts the feedforward needs a separation
constraint supplied from outside the present objective. What was checked is the
list in Table~\ref{tab:safety}; what was not checked includes collision avoidance
under a safety filter, degraded or failed airframes, communication loss, energy
budgets and any physical flight.
...
pathway-ablation and gain-sensitivity receipts form one block; transition values
...
Every extension preserves the paired unit and the evidence-binding pipeline. The
original ten-seed force sweep supplies the exact transition test; the
...
clearance and actuator headroom. Each block has its own declared design,
receipt and equation-defined estimand, so the paper expands the same result by
...
clear deployment requirement: contact-scale separations in a benchmark whose objective carries
no collision constraint, whose targets are drawn with no minimum separation, and
whose physics branch models no inter-airframe downwash. Nothing measured here
constrains collision safety, and an implementation that adopts the feedforward
should carry a separation constraint that the present objective does not contain.
...
The episode-level logs, derived statistical tables, and figure and table code that
produce every number in this manuscript accompany the manuscript source. Each
artifact is recorded with its SHA-256 digest in a manifest, and the figure code
verifies those digests before reading the bytes, so every rebuild is synchronized
with the recorded evidence.

The simulator is a separately available open-source dependency used unmodified apart
from the disturbance injection described in the Methods. The archive supplies all
study-specific code and records needed to reproduce the reported analyses, while the
release ledger records external repository and archive states separately.
\input{generated_availability}

\section*{Statements and declarations}

\paragraph{Funding} No external funding was designated for this study. The work
used local CPU compute and open-source software.

\paragraph{Competing interests} The author declares no financial or non-financial
competing interests relevant to this work.
...
\paragraph{Use of generative AI tools} Generative AI tools assisted with code
scaffolding, analysis scripts and language editing under the author's direction.
The author inspected the simulator implementation, reviewed the analyses and
retains responsibility for the accuracy, integrity and final wording of the work.
All reported numbers are emitted programmatically from digest-bound artifacts,
and the build verifies those artifacts before typesetting.
